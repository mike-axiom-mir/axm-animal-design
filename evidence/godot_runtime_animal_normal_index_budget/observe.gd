...
const EXPECTED_SCHEMA := "axm.runtime-animal-explicit-normal-index-budget-payload/v0.1"
const CONTROL_MODE := "UNINDEXED_EXPLICIT_NORMAL_CONTROL"
const CANDIDATE_MODE := "INDEXED_EXPLICIT_NORMAL_CANDIDATE"
...
func _fail(message: String, out_dir: String) -> void:
    receipt["state"] = "FAIL_RUNTIME_ANIMAL_EXPLICIT_NORMAL_INDEX_BUDGET"
...
func _build_mesh(payload: Dictionary, mode: String) -> Dictionary:
    var raw_positions := payload["positions"] as Array
    var raw_normals := payload["normals"] as Array
    var raw_indices := payload["indices"] as Array
    ...
    if mode == CONTROL_MODE:
        for raw_index in raw_indices:
            ...
            st.set_normal(_gnormal(raw_normals[idx] as Array))
            st.add_vertex(_gvec(raw_positions[idx] as Array))
    elif mode == CANDIDATE_MODE:
        for idx in range(raw_positions.size()):
            st.set_normal(_gnormal(raw_normals[idx] as Array))
            st.add_vertex(_gvec(raw_positions[idx] as Array))
        for raw_index in raw_indices:
    ...
    return {
        ...
        "logical_position_normal_index_bytes": vertex_count * 24 + index_count * 4,
    }
...
func _bounds(raw_positions: Array) -> Dictionary:
    ...
    return {
        ...
        "radius": maxf(size.x, maxf(size.y, size.z)),
    }
...
func _capture(viewport: SubViewport, out_path: String) -> Dictionary:
    ...
    return {
        ...
        "bytes": FileAccess.get_file_as_bytes(out_path).size(),
    }
...
func _initialize() -> void:
    ...
    if payload.get("schema", "") != EXPECTED_SCHEMA:
        ...
        return
    if int(payload.get("source_vertex_count", -1)) != 42 or int(payload.get("source_index_count", -1)) != 240:
        _fail("source count drift", out_dir)
        return
    if int(payload.get("normal_count", -1)) != 42 or String(payload.get("tangent_policy", "")) != "NOT_DEFINED_NO_UV_BASIS":
        _fail("normal/tangent identity drift", out_dir)
        return

    receipt = {
        "schema": "axm.runtime-animal-explicit-normal-index-budget-observation/v0.1",
        "state": "RUNNING",
        "mode": mode,
        "runtime_head": payload.get("runtime_head", ""),
        "exact_geometry_normal_parent_head": payload.get("exact_geometry_normal_parent_head", ""),
        "source_candidate_id": payload.get("source_candidate_id", ""),
        "source_candidate_digest": payload.get("source_candidate_digest", ""),
        "normal_field_id": payload.get("normal_field_id", ""),
        "normal_field_source_candidate_digest": payload.get("normal_field_source_candidate_digest", ""),
        "tangent_policy": payload.get("tangent_policy", ""),
        "truth_boundary": "Exact static right-side Animal Geometry #16 explicit-normal candidate only. A/B changes storage indexing only: same 42 source positions, same 240 source indices / 80 triangles, same 42 explicit logical-quad normals, same material, lighting and cameras. No tangent, UV, deformation, animation, gameplay, final visual preference, arbitrary-mesh safety, target-device or CANON claim.",
    }
    ...
    receipt["state"] = "PASS_RUNTIME_ANIMAL_EXPLICIT_NORMAL_INDEX_OBSERVATION"
    receipt["representation"] = {
        "surface_count": 1,
        "source_vertex_count": int(payload["source_vertex_count"]),
        "source_index_count": int(payload["source_index_count"]),
        "source_triangle_count": int(payload["source_triangle_count"]),
        ...
        "logical_position_normal_index_bytes": int(built["logical_position_normal_index_bytes"]),
        "mesh_build_usec": int(built["build_usec"]),
    }
    receipt["contexts"] = contexts
    receipt["godot_version"] = Engine.get_version_info()
    _write_json(out_dir.path_join("runtime.json"), receipt)
    print("PASS_RUNTIME_ANIMAL_EXPLICIT_NORMAL_INDEX_OBSERVATION")
    quit(0)
