...
const EXPECTED_SCHEMA := "axm.animal-materials-deformed-explicit-normal-review/v0.1"
const EXPECTED_RIGGING_HEAD := "91e2fd01be63df807c035b39f7ec824a4a5a60b8"
const EXPECTED_RIGGING_MODULE_BLOB := "b8df083310727dcd05e7476b2158a081a5c25c8f"
const EXPECTED_NORMAL_HEAD := "79e1667f6cc91e2ec8e41f01df18b6933c9c876d"
const EXPECTED_NORMAL_MODULE_BLOB := "14a1ba3a1e4c96270197f4f449505113f7bf3e6e"
const EXPECTED_TOPOLOGY_HEAD := "bdbb51303bd1b96866b06a71730ccc328bf4f2f6"
const EXPECTED_TOPOLOGY_MODULE_BLOB := "9a0ebcc6169445996756bb446a87e3baf8b9cc33"

var payload: Dictionary
var out_dir := ""
var rigging_head := ""
var rigging_module_blob := ""
var normal_head := ""
var normal_module_blob := ""
var topology_head := ""
var topology_module_blob := ""
...
func _ready() -> void:
    ...
    rigging_head = _arg(args, "--rigging-head")
    rigging_module_blob = _arg(args, "--rigging-module-blob")
    normal_head = _arg(args, "--normal-head")
    normal_module_blob = _arg(args, "--normal-module-blob")
    topology_head = _arg(args, "--topology-head")
    topology_module_blob = _arg(args, "--topology-module-blob")
    if payload_path.is_empty() or out_dir.is_empty():
        push_error("missing --payload or --out-dir")
        get_tree().quit(2)
        return
    if rigging_head != EXPECTED_RIGGING_HEAD or rigging_module_blob != EXPECTED_RIGGING_MODULE_BLOB:
        push_error("unexpected deformed-normal Rigging provenance")
        get_tree().quit(3)
        return
    if normal_head != EXPECTED_NORMAL_HEAD or normal_module_blob != EXPECTED_NORMAL_MODULE_BLOB:
        push_error("unexpected explicit-normal provenance")
        get_tree().quit(4)
        return
    if topology_head != EXPECTED_TOPOLOGY_HEAD or topology_module_blob != EXPECTED_TOPOLOGY_MODULE_BLOB:
        push_error("unexpected topology provenance")
        get_tree().quit(5)
        return
    ...
    if payload.get("schema", "") != EXPECTED_SCHEMA:
        ...
        get_tree().quit(6)
        return
    if payload.get("exact_rigging_donor_head", "") != rigging_head:
        push_error("payload Rigging head mismatch")
        get_tree().quit(7)
        return
    if payload.get("exact_rigging_module_blob", "") != rigging_module_blob:
        push_error("payload Rigging module blob mismatch")
        get_tree().quit(8)
        return
    if payload.get("exact_normal_donor_head", "") != normal_head:
        push_error("payload explicit-normal donor head mismatch")
        get_tree().quit(9)
        return
    if payload.get("exact_normal_module_blob", "") != normal_module_blob:
        push_error("payload explicit-normal module blob mismatch")
        get_tree().quit(10)
        return
    if payload.get("exact_topology_donor_head", "") != topology_head:
        push_error("payload topology donor head mismatch")
        get_tree().quit(11)
        return
    if payload.get("exact_topology_module_blob", "") != topology_module_blob:
        push_error("payload topology module blob mismatch")
        get_tree().quit(12)
        return
        ...
        for normal_mode in payload["normal_modes"]:
            for context in payload["camera_contexts"]:
                ...
                for variant_name in ["historical_right", "exact_mirror_right"]:
                    _set_mesh(pose, variant_name, normal_mode)
                    await get_tree().process_frame
                    await get_tree().process_frame
                    var filename := "%s__%s__%s__%s.png" % [pose_file_id, normal_mode, context, variant_name]
    ...
    var telemetry := {
        "schema": "axm.animal-materials-deformed-explicit-normal-review-telemetry/v0.1",
        ...
        "rigging_module_blob": rigging_module_blob,
        "normal_donor_head": normal_head,
        "normal_module_blob": normal_module_blob,
        "topology_donor_head": topology_head,
        "topology_module_blob": topology_module_blob,
        ...
        "truth_boundary": {
            ...
            "representative_deformed_shading_is_captured": true,
            "continuous_deformation_visual_quality_is_proven": false,
            "tangents_or_uvs_are_tested": false,
            "production_skin_normal_transport_is_tested": false,
    ...
    file.close()
    print("PASS_TARGET_HOST_DEFORMED_EXPLICIT_NORMAL_REVIEW_CAPTURED")
    get_tree().quit(0)
...
func _to_godot_normal(raw: Array) -> Vector3:
    return Vector3(float(raw[0]), float(raw[2]), -float(raw[1])).normalized()

func _set_mesh(pose: Dictionary, variant_name: String, normal_mode: String) -> void:
    var raw_positions: Array = pose["positions"]
    var indices: Array = payload["topologies"][variant_name]
    var positions: Array[Vector3] = []
    for raw in raw_positions:
        positions.append(_to_godot(raw))

    var selected_normals: Array[Vector3] = []
    if normal_mode == "vertex_generated":
        selected_normals.resize(positions.size())
        for i in range(selected_normals.size()):
            selected_normals[i] = Vector3.ZERO
        for offset in range(0, indices.size(), 3):
            ...
            selected_normals[ia] += face_area_normal
            selected_normals[ib] += face_area_normal
            selected_normals[ic] += face_area_normal
        for i in range(selected_normals.size()):
            selected_normals[i] = selected_normals[i].normalized()
    elif normal_mode == "logical_quad_explicit":
        var raw_normals: Array = pose["explicit_normals"]
        if raw_normals.size() != positions.size():
            push_error("explicit normal count does not match position count")
            get_tree().quit(16)
            return
        for raw in raw_normals:
            selected_normals.append(_to_godot_normal(raw))
    else:
    ...
    for offset in range(0, indices.size(), 3):
        ...
        var ic := int(indices[offset + 2])
        for idx in [ia, ib, ic]:
            st.set_normal(selected_normals[idx])
