#!/usr/bin/env python3
"""Verify bilateral Animal UV seam / texture-filter real-render evidence."""
...
EXPECTED_PAYLOAD_SCHEMA = "axm.animal-materials-tangent-space-lookdev/v0.1"
EXPECTED_TELEMETRY_SCHEMA = "axm.animal-materials-texture-seam-filter-bilateral-lookdev-telemetry/v0.1"
...
EXPECTED_PREDECESSOR_MATERIALS_HEAD = "edec05e2e803dd3c7483e374c12eb1fa5a9c5501"
PASS_STATE = "PASS_ANIMAL_BILATERAL_PERIODIC_NORMAL_TEXTURE_SEAM_FILTER_LOCALIZATION"
HOLD_STATE = "HOLD_ANIMAL_BILATERAL_TEXTURE_SEAM_FILTER_LOOKDEV"
...
def main() -> int:
    ...
    parser.add_argument("--predecessor-materials-head", required=True)
    parser.add_argument("--out", required=True, type=Path)
    args = parser.parse_args()

    payload = _load(args.payload)
    telemetry = _load(args.telemetry)
    if args.predecessor_materials_head != EXPECTED_PREDECESSOR_MATERIALS_HEAD:
        raise SystemExit("predecessor Materials head drift")
    ...
    if payload.get("exact_rigging_tangent_head") != EXPECTED_RIGGING_TANGENT_HEAD:
        raise SystemExit("Rigging tangent payload drift")
    if payload.get("exact_geometry_uv_tangent_head") != EXPECTED_GEOMETRY_HEAD:
        raise SystemExit("Geometry UV/tangent payload drift")
    if telemetry.get("rigging_tangent_head") != EXPECTED_RIGGING_TANGENT_HEAD:
        raise SystemExit("Rigging tangent telemetry drift")
    if telemetry.get("geometry_uv_tangent_head") != EXPECTED_GEOMETRY_HEAD:
        raise SystemExit("Geometry UV/tangent telemetry drift")
    if int(payload.get("budget", {}).get("side_count", 0)) != 2:
        raise SystemExit("expected bilateral payload")
    if int(payload.get("budget", {}).get("representative_pose_count", 0)) != 20:
        raise SystemExit("expected 20 bilateral representative poses")
    if telemetry.get("render_count") != 200:
        raise SystemExit(f"expected 200 real renders, got {telemetry.get('render_count')}")
    ...
    side_contexts = {"left": 0, "right": 0}
    side_localized = {"left": 0, "right": 0}
    side_negative_changed = {"left": [], "right": []}
    side_mip_changed = {"left": [], "right": []}
    side_overlap = {"left": [], "right": []}

    for pose in payload["pose_sets"]:
        pose_id = str(pose["pose_id"])
        side = str(pose["side"])
        if side not in side_contexts:
            raise SystemExit(f"unexpected side: {side}")
        for context in payload["camera_contexts"]:
            side_contexts[side] += 1
            paths = {
            ...
            if locator_pixels >= 20:
                ...
                if candidate_vs_negative["changed_pixels_gt_1_lsb"] > 0:
                    ...
                    if overlap >= 0.50:
                        ...
                        side_localized[side] += 1

            side_negative_changed[side].append(candidate_vs_negative["changed_pixels_gt_1_lsb"])
            side_mip_changed[side].append(candidate_vs_no_mip["changed_pixels_gt_1_lsb"])
            side_overlap[side].append(overlap)
            rows.append({
                "pose_id": pose_id,
                "side": side,
                "weighting": str(pose["weighting"]),
    ...
    expected_contexts = len(payload["pose_sets"]) * len(payload["camera_contexts"])
    bilateral_coverage_complete = (
        side_contexts == {"left": 20, "right": 20}
        and visible_seam_contexts == expected_contexts
        and side_localized == {"left": 20, "right": 20}
    )
    localized_negative_valid = (
        visible_seam_contexts == expected_contexts
        and negative_visible_contexts == expected_contexts
        and localized_contexts == expected_contexts
    )
    state = PASS_STATE if (
        ...
        and bilateral_coverage_complete
    ) else HOLD_STATE

    side_summary = {}
    for side in ("left", "right"):
        side_summary[side] = {
            "context_count": side_contexts[side],
            "localized_negative_context_count": side_localized[side],
            "negative_changed_pixels_min": min(side_negative_changed[side]),
            "negative_changed_pixels_max": max(side_negative_changed[side]),
            "mip_changed_pixels_min": min(side_mip_changed[side]),
            "mip_changed_pixels_max": max(side_mip_changed[side]),
            "minimum_localization_overlap": min(side_overlap[side]),
        }

    summary = {
        "state": state,
        "materials_head": args.materials_head,
        "predecessor_materials_head": args.predecessor_materials_head,
        "renderer": {
        ...
        "gates": {
            ...
            "expected_bilateral_contexts": expected_contexts,
            ...
            "bilateral_coverage_complete": bilateral_coverage_complete,
            "mip_filter_difference_visible_contexts": mip_filter_visible_contexts,
        },
        "side_summary": side_summary,
        "comparisons": rows,
        "provenance": {
            "rigging_tangent_head": payload.get("exact_rigging_tangent_head"),
            "geometry_uv_tangent_head": payload.get("exact_geometry_uv_tangent_head"),
            "rig_donor_head": payload.get("exact_rig_donor_head"),
            "prior_right_transport_texture_filter_evidence_head": EXPECTED_PREDECESSOR_MATERIALS_HEAD,
        },
        "truth_boundary": {
            "bilateral_real_renderer_sampling_proven": bilateral_coverage_complete,
            "base_level_periodic_texture_edge_closure_proven": candidate_edge_closed,
            "real_mipped_sampling_exercised": True,
            "left_right_pixel_equality_claimed": False,
            ...
            "technical_art_transport_or_runtime_adoption_claimed": False,
            "continuous_playback_quality_claimed": False,
    ...
    if not candidate_response_visible:
        raise SystemExit("periodic normal texture did not produce visible target-host response in every bilateral context")
    if not localized_negative_valid:
        raise SystemExit("edge-mutated seam negative was not visible and localized in every bilateral context")
    if not bilateral_coverage_complete:
        raise SystemExit("bilateral seam/filter coverage incomplete")
    return 0
