#!/usr/bin/env python3
"""Rebind the unchanged Animal articulation clip to the exact mirror-surface Rigging successor.

Animation consumes Rigging PR #15 exactly, preserves the existing smoothstep-v0 clip,
samples both selected-003 front elbows at all 41 authored times, checks exact bilateral
posed-vertex and retained surface-metric symmetry, and emits the right exact-mirror
surface into the existing unshaded Godot discrete-sample proof host.

This does not accept the newer explicit normal-field candidate, visual quality,
continuous interpolation, runtime-controller behavior, gameplay, CANON, or production
readiness.
"""
...
PREVIOUS_RIGGING_HEAD = "94bc573e2e06ba7a35c9908c141e2f939d4739a8"
NEWER_NORMAL_FIELD_HEAD = "79e1667f6cc91e2ec8e41f01df18b6933c9c876d"
LEFT_SUCCESSOR_DIGEST = "ace2366d8cd14c00df670b5fe1f0780ab2d01992482455ad5f7c4c9cadffeeba"
RIGHT_MIRROR_SURFACE_DIGEST = "086ffe6f48af0cc3506871a4754b7395ba4c1fa3155eb4f420854a37e92181db"
...
GATE = "PASS_BILATERAL_EXACT_MIRROR_SURFACE_41_SAMPLE_MOTION_REBIND"
SCHEMA = "axm.animal-animation-bilateral-mirror-surface-motion-rebind/v0.1"
FRAME_SCHEMA = "axm.animal-animation-bilateral-mirror-surface-motion-frames/v0.1"
...
def sample_side(
    ...
    for sample_index in range(41):
        ...
        rounded_positions = [
            [round(float(value), 12) for value in point] for point in metrics["positions"]
        ]
        frames.append(
            {
                ...
                "maximum_triangle_area_ratio": float(metrics["maximum_triangle_area_ratio"]),
                "minimum_edge_length_ratio": float(metrics["minimum_edge_length_ratio"]),
                "maximum_edge_length_ratio": float(metrics["maximum_edge_length_ratio"]),
                "collapsed_triangles": int(metrics["collapsed_triangles"]),
                "nonadjacent_self_intersection_pairs": int(
                    metrics["nonadjacent_self_intersection_pairs"]
                ),
                "fixed_weight_vertex_max_drift_m": float(
                    metrics["fixed_weight_vertex_max_drift_m"]
                ),
                "rigid_weight_radius_max_drift_m": float(
                    metrics["rigid_weight_radius_max_drift_m"]
                ),
            }
        )
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.rigging_head != RIGGING_HEAD or git_head(rigging_root) != RIGGING_HEAD:
        raise ValueError("mirror-surface Rigging donor head drift")
    ...
    from axm_animal_design import bilateral_mirror_surface_rigging_rebind as mirror_rig  # type: ignore
    ...
    from axm_animal_design.bilateral_mirror_surface_topology import (  # type: ignore
        inspect_exact_mirror_surface_repair,
    )

    if mirror_rig.GEOMETRY_MIRROR_SURFACE_HEAD != GEOMETRY_HEAD:
        raise ValueError("Geometry identity drift inside mirror-surface Rigging donor")
    if mirror_rig.ORGANIC_BILATERAL_HEAD != ORGANIC_HEAD:
        raise ValueError("Organic identity drift inside mirror-surface Rigging donor")
    if mirror_rig.PREVIOUS_BILATERAL_RIGGING_HEAD != PREVIOUS_RIGGING_HEAD:
        raise ValueError("previous bilateral Rigging lineage drift")
    if mirror_rig.RIG_DONOR_HEAD != RIG_DONOR_HEAD:
        raise ValueError("rig-plan donor identity drift inside mirror-surface Rigging donor")
    ...
    bilateral_profile = load_json(
        rigging_root / "examples/quadruped_elbow_bilateral_successor_003.json"
    )
    plan = load_json(rig_donor_root / "examples/quadruped_rig_probe_001.json")
    weighting_profile = load_json(
        rig_donor_root / "examples/quadruped_weighting_refinement_001.json"
    )
    ...
    rigging_receipt = mirror_rig.inspect_bilateral_mirror_surface_rigging_rebind(
        spec, left_profile, bilateral_profile, plan, weighting_profile
    )
    if rigging_receipt.get("state") != mirror_rig.PASS_STATE:
        raise ValueError("mirror-surface Rigging prerequisite is not a PASS")
    ...
    if mirror.get("gate") != "PASS_EXACT_MIRRORED_VERTEX_AND_SURFACE_METRICS":
        raise ValueError("mirror-surface Rigging exact bilateral gate is not green")
    if float(mirror["maximum_mirrored_pose_residual_m"]) > 1e-12:
        raise ValueError("mirror-surface Rigging posed-vertex residual drift")
    if float(mirror["maximum_structural_metric_residual"]) > 1e-12:
        raise ValueError("mirror-surface Rigging structural-metric residual drift")
    ...
    if geometry.get("state") != "PASS_BILATERAL_EXACT_MIRROR_SURFACE_TOPOLOGY_REPAIR":
        raise ValueError("exact mirror-surface Geometry prerequisite is not green")
    ...
    if right_digest != RIGHT_MIRROR_SURFACE_DIGEST:
        raise ValueError(f"right exact-mirror successor digest drift: {right_digest}")
    ...
    pairs = mirror_rig._vertex_pairs(left_candidate)
    metric_keys = (
    ...
    max_surface_metric_residual = 0.0
    for left_frame, right_frame in zip(left_frames, right_frames):
        ...
        for left_index, right_index in pairs:
            ...
            max_mirror_residual = max(
                max_mirror_residual, math.dist((lx, -ly, lz), (rx, ry, rz))
            )
        for key in metric_keys:
            max_surface_metric_residual = max(
                max_surface_metric_residual,
                abs(float(left_frame[key]) - float(right_frame[key])),
            )
    if max_mirror_residual > 1e-10:
        raise ValueError(f"Animation bilateral posed-vertex mirror drift: {max_mirror_residual}")
    if max_surface_metric_residual > 1e-10:
        raise ValueError(
            f"Animation bilateral surface-metric mirror drift: {max_surface_metric_residual}"
        )
    ...
    max_intersections = max(
        frame["nonadjacent_self_intersection_pairs"] for frame in right_frames
    )
    if max_collapsed != 0 or max_intersections != 0:
        raise ValueError("right exact-mirror Animation samples introduce structural defects")
    ...
    (args.out / "bilateral_mirror_surface_motion_frames.json").write_text(
        json.dumps(frames_doc, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    ...
    receipt = {
        ...
        "source_identity": {
            ...
            "geometry_mirror_surface_head": GEOMETRY_HEAD,
            "bilateral_mirror_surface_rigging_head": RIGGING_HEAD,
            "previous_bilateral_rigging_head": PREVIOUS_RIGGING_HEAD,
            "rig_plan_donor_head": RIG_DONOR_HEAD,
            "rig_plan_digest": RIG_PLAN_DIGEST,
            "left_successor_digest": left_digest,
            "right_exact_mirror_successor_digest": right_digest,
        ...
        "observed": {
            ...
            "maximum_bilateral_surface_metric_residual": max_surface_metric_residual,
            "maximum_right_collapsed_triangles": max_collapsed,
            "maximum_right_nonadjacent_self_intersection_pairs": max_intersections,
            "exact_right_neutral_start_return": right_frames[0]["positions"]
            == right_frames[40]["positions"],
            "right_peak_sample_index": 20,
            "right_peak_angle_deg": right_frames[20]["angle_deg"],
        },
        "dependency_boundary": {
            "consumed_topology_identity": GEOMETRY_HEAD,
            "consumed_rigging_identity": RIGGING_HEAD,
            "newer_explicit_normal_field_known": NEWER_NORMAL_FIELD_HEAD,
            "newer_explicit_normal_field_consumed": False,
            "target_host_surface_mode": "UNSHADED_TRIANGLE_GEOMETRY",
            "reason": (
                "Animation closes the exact Geometry #13 + Rigging #15 motion/playback "
                "rebind only. Geometry #16 normal-field evidence remains a separate "
                "Materials/Visual-QA/Art-Direction dependency and is not consumed by "
                "this unshaded proof host."
            ),
        },
        "truth": {
            "proves": [
                "the unchanged bilateral clip can be applied directly to Geometry PR #13's exact-mirror selected-003 surface through Rigging PR #15",
                "all 41 authored right-elbow samples remain structurally green under the exact smoothstep-v0 Rigging implementation",
                "left and right posed vertex fields and retained surface deformation extrema remain mirrored at all 41 authored Animation sample times",
                "the exact-mirror right surface can be emitted to the existing unshaded Godot discrete-sample proof host without changing motion variables",
            ],
            "does_not_prove": [
                "visual acceptance of generated or explicit normals/tangents",
                "Animation acceptance of Geometry PR #16 or later normal-field successors",
    ...
    (args.out / "bilateral_mirror_surface_motion_receipt.json").write_text(
    ...
    payload = {
        ...
        "proof_scope": "PINNED_GODOT_EXACT_MIRROR_SURFACE_AUTHORED_SAMPLE_APPLICATION_NOT_REALTIME_CONTROLLER",
        ...
        "topology": {
            ...
            "geometry_head": GEOMETRY_HEAD,
            "rigging_head": RIGGING_HEAD,
            "explicit_normal_field_consumed": False,
            "rendering_scope": "UNSHADED_GEOMETRY_REACHABILITY_AND_READBACK_ONLY",
        },
    ...
    summary = {
        "schema": "axm.animal-animation-bilateral-mirror-surface-godot-payload-summary/v0.1",
        "gate": "PASS_BILATERAL_EXACT_MIRROR_SURFACE_GODOT_PAYLOAD_BUILD",
        "payload_sha256": sha256(payload_bytes),
        "source_identity": receipt["source_identity"],
        "source_playback": payload["source_playback"],
        "topology": payload["topology"],
        "unique_authored_surface_digests": len(
            {frame["surface_digest"] for frame in display_frames}
        ),
        "bounds_godot": payload["bounds_godot"],
        "dependency_boundary": receipt["dependency_boundary"],
    }
    (args.payload_dir / "quadruped_animation_payload_summary.json").write_text(
